...
The endpoint provides health, byte-bounded document listing, single-document reads, and merge-only document upserts. Oversized single-document data is returned as resumable base64-encoded JSON byte chunks. User profiles and audit logs are read-only. Access is limited to Ditto's known production document roots and the `tasks`, `documents`, and `vendors` family subcollections.
